...
class DataTrainingArguments:
    ...
    conll_file: str = field(default="/users/the/NER_MTB/my_biobert/data/tokcls")
    output_file: str = field(default="/users/the/NER_MTB/my_biobert/data/tokcls")
...
def main():
    ...
    if os.path.isdir(data_args.output_file) and training_args.do_train and not training_args.overwrite_output_file:
        last_checkpoint = get_last_checkpoint(data_args.output_file)
        if last_checkpoint is None and len(os.listdir(data_args.output_file)) > 0:
            raise ValueError(
                f"Output directory ({data_args.output_file}) already exists and is not empty. "
                "Use --overwrite_output_file to overcome."
            )
        elif last_checkpoint is not None and training_args.resume_from_checkpoint is None:
            logger.info(
                f"Checkpoint detected, resuming training at {last_checkpoint}. To avoid this behavior, change "
                "the `--output_file` or add `--overwrite_output_file` to train from scratch."
            )
    ...
    if training_args.do_predict:
        ...
        logger.debug(f"predict_dataset {predict_dataset}, predict_dataset.features {predict_dataset.features}")
    ...
    # helen added: readable test pred file
        ...
        for i in range(batch_size):
            for j in range(seq_len):
                if label_ids[i, j] != nn.CrossEntropyLoss().ignore_index:
                    out_label_list[i].append(label_map[label_ids[i][j]]) # true label id
                    preds_list[i].append(label_map[preds[i][j]]) # pred label id
    ...
    if training_args.do_predict:
        ...
        predictions, label_ids, metrics = results
        # predictions = results.predictions
        # metrics = results.metrics
        logger.info(label_ids)
        preds_lst, out_label_lst  = align_predictions(predictions, label_ids)
        # metrics["test_samples"] = len(predict_dataset)
        ...
        # #               open(f"{output_file}/test_outputs.json", "w"))
        input_conll_test_file = data_args.conll_file
        with open(output_file, "w") as writer:
            with open(input_conll_test_file, "r") as f:
                example_id = 0
                for line in f:
                    if line == "" or line == "\n":
                        writer.write(line)
                        # not understand
                        if not preds_lst[example_id]:
                            example_id += 1
                    elif preds_lst[example_id]:
                        entity_label = preds_lst[example_id].pop(0)
                        true_label = out_label_lst[example_id].pop(0)
                        # # if entity_label == "O":
                        #     output_line = line.split()[0] +"\t"+entity_label + "\t"+true_label+"\n"
                        # else:
                        #     output_line = line.split()[0] + "\t"+entity_label[0] +"\t"+true_label+"\n"
                        output_line = line.split()[0] +"\t"+entity_label + "\t"+true_label+"\n"
